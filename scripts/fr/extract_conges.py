...
# ---- topic sections ----
# A. Troisieme partie > Livre Ier : Duree du travail, repos et conges
#    Titre IV : Conges payes et autres conges (L3141-x, L3142-x)
#    Titre V   : Compte epargne-temps (L3151-x, L3152-x, L3153-x) -- separate Titre, sibling of Titre IV
LEG_TITRE_IV_CONGES = "LEGISCTA000006160756"
LEG_TITRE_V_CET = "LEGISCTA000006160757"
REG_TITRE_IV_CONGES = "LEGISCTA000018534321"
REG_TITRE_V_CET = "LEGISCTA000018534073"

# B. Premiere partie > Livre II : Le contrat de travail > Titre II : Formation et execution du contrat de travail
#    Chapitre V : Maternite, paternite, adoption et education des enfants (L1225-x)
...
# Note: the réglementaire (D) articles for compte épargne-temps use a different
# chapter number (3154) than the législatif (L3151-L3153) ones, even though they sit
# under the same Titre V : Compte épargne-temps node -- include 3154 to capture them.
# Also: Titre IV (Congés payés et autres congés) has a "Chapitre III : Dispositions
# pénales" whose législatif articles are all abrogated (no in-force L3143-x remain)
# but whose réglementaire articles (R3143-x) are still in force -- include 3143.
ALLOWED_PREFIXES = {"3141", "3142", "3143", "3151", "3152", "3153", "3154", "1225"}
...
leg_node_congespayes = find(root, LEG_TITRE_IV_CONGES)
leg_node_cet = find(root, LEG_TITRE_V_CET)
reg_node_congespayes = find(root, REG_TITRE_IV_CONGES)
reg_node_cet = find(root, REG_TITRE_V_CET)

leg_node_maternite = find(root, LEG_CHAP_V_MATERNITE)
reg_node_maternite = find(root, REG_CHAP_V_MATERNITE)

BOOK_PATH_A = ["Troisième partie : Durée du travail, salaire, intéressement, participation et épargne salariale",
               "Livre Ier : Durée du travail, repos et congés"]
BOOK_PATH_B = ["Première partie : Les relations individuelles de travail",
               "Livre II : Le contrat de travail",
               "Titre II : Formation et exécution du contrat de travail"]

leg_sections_a = (
    collect_sections_from_children(leg_node_congespayes, BOOK_PATH_A + ["Titre IV : Congés payés et autres congés"])
    + collect_sections_from_children(leg_node_cet, BOOK_PATH_A + ["Titre V : Compte épargne-temps"])
)
reg_sections_a = (
    collect_sections_from_children(reg_node_congespayes, BOOK_PATH_A + ["Titre IV : Congés payés et autres congés"])
    + collect_sections_from_children(reg_node_cet, BOOK_PATH_A + ["Titre V : Compte épargne-temps"])
)

# Chapitre V itself holds sub-sections only (no direct articles at chapter level in the
# samples inspected), but call collect_sections_from_children on the chapter node's own
# parent-style iteration: since collect_sections_from_children iterates over node's
# children, and the chapter node IS what we want to scan (its children are sections),
# we pass the chapter node directly.
leg_sections_b = collect_sections_from_children(
    leg_node_maternite, BOOK_PATH_B + ["Chapitre V : Maternité, paternité, adoption et éducation des enfants"]
)
reg_sections_b = collect_sections_from_children(
    reg_node_maternite, BOOK_PATH_B + ["Chapitre V : Maternité, paternité, adoption et éducation des enfants"]
)

all_sections = leg_sections_a + reg_sections_a + leg_sections_b + reg_sections_b
...
output = {
    ...
    "topic": "Congés payés, autres congés, maternité et paternité",
    "code_totals": {
        "articles_L": code_L,
        "articles_R": code_R,
        "articles_D": code_D,
...
OUT = "/Users/joachimndoye/alleger-le-code/data/fr_conges.json"
