...
def clean_html_to_text(s):
    if not s:
        return ""
    # normalize breaks/paragraphs to \n
    s = re.sub(r'(?i)<br\s*/?>', '\n', s)
    s = re.sub(r'(?i)</p\s*>', '\n\n', s)
    s = re.sub(r'(?i)<p[^>]*>', '', s)
    s = re.sub(r'(?i)</?li[^>]*>', '\n', s)
...
# ---- topic sections ----
LEG_ID = "LEGISCTA000006160710"  # Partie législative > ... > Titre III
REG_ID = "LEGISCTA000018537734"  # Partie réglementaire > ... > Titre III

leg_node = find(root, LEG_ID)
reg_node = find(root, REG_ID)
...
def collect_sections(node, path_prefix):
    """
    Walk a section node's children. Build list of 'sections' where each
    section corresponds to a leaf-ish grouping that directly contains articles
    (a chapter or a section-level node with articles as direct children),
    preserving hierarchy order. We flatten: for every node that has at least
    one direct article child, emit one section entry with those articles.
    Recurse into non-article children regardless.
    """
    sections = []
...
leg_sections = collect_sections(leg_node, ["Livre II : Le contrat de travail", "Titre III : Rupture du contrat de travail à durée indéterminée"])
reg_sections = collect_sections(reg_node, ["Livre II : Le contrat de travail", "Titre III : Rupture du contrat de travail à durée indéterminée"])
...
output = {
    ...
    "topic": "Rupture du contrat de travail à durée indéterminée",
    "code_totals": {
        "articles_L": code_L,
        "articles_R": code_R,
        "articles_D": code_D,
...
with open("fr_licenciement_out.json", "w", encoding="utf-8") as f:
    json.dump(output, f, ensure_ascii=False, indent=2)

print("WROTE fr_licenciement_out.json")
